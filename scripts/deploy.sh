...
until docker exec indian_store_postgres pg_isready -U "${DB_USER:-store_admin}" -d "${DB_NAME:-indian_store_db}" > /dev/null 2>&1; do
  sleep 2
done

# 4. Run database migrations inside API container
echo "🔄 Running Database Schema Migrations..."
docker exec -i indian_store_postgres psql -U "${DB_USER:-store_admin}" -d "${DB_NAME:-indian_store_db}" < src/database/schema.sql
